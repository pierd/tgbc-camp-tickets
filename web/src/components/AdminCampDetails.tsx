...
import { DbCollections, type DbCamp, type DbCampParticipant } from 'shared';
import { useStreamDocument, useFirebaseQuery, queryT, orderByT } from '../firebaseHooks';
...
export const AdminCampDetails: React.FC = () => {
  ...
  // Get camp details
  const campRef = doc(db, DbCollections.camps, campId!) as DocumentReference<DbCamp, DbCamp>;
  const campData = useStreamDocument<DbCamp, DbCamp>(campRef);

  // Get participants for this camp
  const participantsRef = collection(db, DbCollections.camps, campId!, DbCollections.campParticipants) as CollectionReference<DbCampParticipant, DbCampParticipant>;
  const participantsQuery = queryT(participantsRef, orderByT('createdAt', 'desc'));
      ...
      <header className="admin-header">
        <div className="admin-header-content">
          ...
          <div className="admin-nav">
            <Link to="/admin" className="nav-link">← Back to Admin</Link>
            <button
        ...
        <div className="camp-details-section">
          ...
          <div className="camp-details-grid">
            ...
            <div className="detail-card">
              <h3>Pricing</h3>
              <div className="detail-item">
                <strong>Initial Installment:</strong> {formatCurrency(camp.initialInstallmentCents)}
              </div>
              <div className="detail-item">
                <strong>Installment Amount:</strong> {formatCurrency(camp.installmentCents)}
              </div>
              <div className="detail-item">
                <strong>Total Cost:</strong> {formatCurrency(camp.totalCostCents)}
              </div>
              <div className="detail-item">
                <strong>Out of State Rebate:</strong> {formatCurrency(camp.outOfStateRebateCents)}
              </div>
              <div className="detail-item">
                <strong>In State Extra Cost:</strong> {formatCurrency(camp.inStateExtraCostCents)}
              </div>
            </div>
          </div>
        </div>
                  ...
                  <div key={participantDoc.id} className="participant-card">
                    <div className="participant-info">
                      ...
                      <div className="participant-state">
                        <strong>State:</strong> {campStateDisplayName[participant.state as keyof typeof campStateDisplayName]}
                      </div>
                      <div className="participant-joined">
                        <strong>Joined:</strong> {formatDate(participant.createdAt)}
                      </div>
                    </div>
                  </div>
                );
