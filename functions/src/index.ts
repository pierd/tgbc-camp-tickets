...
import { FieldValue, Timestamp, UpdateData, WithFieldValue } from "firebase-admin/firestore";
...
export const joinCamp = onCall<JoinCampRequest>({ cors: true }, async (request) => {
  ...
  const { campId, returnUrl, state } = request.data;
...
export const payInstallment = onCall<PayInstallmentRequest>({ cors: true }, async (request) => {
  ...
  const { campId, returnUrl, installmentCount } = request.data;
  return initiatePayment(request.auth.uid, campId, returnUrl, { isInitialInstallment: false, installmentCount });
});

async function initiatePayment(userId: string, campId: string, returnUrl: string, options: { isInitialInstallment: true; state: CampState } | { isInitialInstallment: false; installmentCount: number }) {
  ...
  const participantRef = collection<DbCampParticipant>(DbCollections.campParticipants).doc(getCampParticipantId({ userId, campId }));
  const participant = await participantRef.get();
  const fetchedParticipantData = participant.data();
  let participantData: Omit<DbCampParticipant, "createdAt" | "updatedAt"> & WithFieldValue<{ createdAt: Timestamp; updatedAt: Timestamp }>;
  if (fetchedParticipantData !== undefined) {
    participantData = fetchedParticipantData;
    if (options.isInitialInstallment) {
      throw new HttpsError("invalid-argument", "User has already joined the camp");
    }
  } else {
    if (!options.isInitialInstallment) {
  ...
  const participantCostCents = calculateParticipantCostCents(campData, participantData.state);
  if (participantData.paidCents >= participantCostCents) {
    throw new HttpsError("invalid-argument", "User has already paid for the camp");
  }
  ...
  await collection<DbStripeCheckoutSession>(DbCollections.stripeCheckoutSessions).doc(session.id).set({
    createdAt: FieldValue.serverTimestamp(),
...
async function fulfillCheckoutSession(session: Stripe.Checkout.Session, successEvent: boolean) {
  logger.info("Fulfilling checkout session", session);
  const sessionRef = collection<DbStripeCheckoutSession>(DbCollections.stripeCheckoutSessions).doc(session.id);
  await db.runTransaction(async (transaction) => {
    ...
    if (!failed) {
      ...
      const campRef = collection<DbCamp>(DbCollections.camps).doc(sessionData.campId);
      const camp = await campRef.get();
      const campData = camp.data();
      if (!campData) {
        throw new HttpsError("not-found", "Camp not found");
      ...
      logger.info("Creating participant installment", sessionData.userId, session.id);
      const participantRef = collection<DbCampParticipant>(DbCollections.campParticipants).doc(getCampParticipantId({ userId: sessionData.userId, campId: sessionData.campId }));
      ...
      transaction.set(participantRef.collection<DbCampParticipantInstallment>(DbCollections.installments).doc(session.id).ref, {
        createdAt: FieldValue.serverTimestamp(),
